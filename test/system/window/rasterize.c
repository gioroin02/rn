...
void paintSegment(PxBitmap* bitmap, S64Vec2 p0, S64Vec2 p1, U8Vec4 c0, U8Vec4 c1)
{
    ...
    if (max_x - min_x > max_y - min_y) {
        ...
        for (index = 0; index <= delta_x; index += 1) {
            ...
            U8Vec4 color = u8Vec4(
                pxClamp(c0.r + alpha * (c1.r - c0.r), 0, 255),
                pxClamp(c0.g + alpha * (c1.g - c0.g), 0, 255),
                pxClamp(c0.b + alpha * (c1.b - c0.b), 0, 255),
                pxClamp(c0.a + alpha * (c1.a - c0.a), 0, 255));
    ...
    for (point.y = min_y; point.y <= max_y; point.y += 1) {
        for (point.x = min_x; point.x <= max_x; point.x += 1) {
            ...
            if ((w0 | w1 | w2) >= 0) {
                ...
                U8Vec4 color = u8Vec4(
                    pxClamp(alpha * c0.r + beta * c1.r + gamma * c2.r, 0, 255),
                    pxClamp(alpha * c0.g + beta * c1.g + gamma * c2.g, 0, 255),
                    pxClamp(alpha * c0.b + beta * c1.b + gamma * c2.b, 0, 255),
                    pxClamp(alpha * c0.a + beta * c1.a + gamma * c2.a, 0, 255));
