#ifndef PX_WIN32_BITMAP_C
#define PX_WIN32_BITMAP_C

#include "bitmap.h"

PxWin32Bitmap* pxWin32BitmapReserve(PxMemoryArena* arena)
{
    return pxMemoryArenaReserveOneOf(arena, PxWin32Bitmap);
}

b32 pxWin32BitmapCreate(PxWin32Bitmap* self, PxMemoryArena* arena, ssize width, ssize height)
{
    pxMemorySet(self, sizeof* self, 0xAB);

    if (width <= 0 || height <= 0 || width > PX_SSIZE_MAX / height) return 0;

    u32* pntr = pxMemoryArenaReserveManyOf(arena, u32, width * height);

    if (pntr != PX_NULL) {
        self->info.bmiHeader.biSize        = sizeof (BITMAPINFOHEADER);
        self->info.bmiHeader.biWidth       = width;
        self->info.bmiHeader.biHeight      = -height;
        self->info.bmiHeader.biPlanes      = 1;
        self->info.bmiHeader.biBitCount    = 32;
        self->info.bmiHeader.biCompression = BI_RGB;

        self->pntr   = pntr;
        self->width  = width;
        self->height = height;

        pxWin32BitmapFill(self, 0, 0, 0, 0);

        return 1;
    }

    return 0;
}

ssize pxWin32BitmapWidth(PxWin32Bitmap* self)
{
    return self->width;
}

ssize pxWin32BitmapHeight(PxWin32Bitmap* self)
{
    return self->height;
}

void pxWin32BitmapFill(PxWin32Bitmap* self, u8 red, u8 green, u8 blue, u8 alpha)
{
    u32 color = ((u32) blue)  << 0
              | ((u32) green) << 8
              | ((u32) red)   << 16
              | ((u32) alpha) << 24;

    ssize row = 0;
    ssize col = 0;

    for (row = 0; row < self->height; row += 1) {
        for (col = 0; col < self->width; col += 1)
            self->pntr[self->width * row + col] = color;
    }
}

b32 pxWin32BitmapPixelSet(PxWin32Bitmap* self, ssize x, ssize y, u8 red, u8 green, u8 blue, u8 alpha)
{
    if (x < 0 || x >= self->width) return 0;
    if (y < 0 || y >= self->height) return 0;

    u32 color = ((u32) blue)  << 0
              | ((u32) green) << 8
              | ((u32) red)   << 16
              | ((u32) alpha) << 24;

    self->pntr[self->width * y + x] = color;

    return 1;
}

b32 pxWin32BitmapPixelGet(PxWin32Bitmap* self, ssize x, ssize y, u8* red, u8* green, u8* blue, u8* alpha)
{
    if (x < 0 || x >= self->width) return 0;
    if (y < 0 || y >= self->height) return 0;

    u32 color = self->pntr[self->width * y + x];

    if (red != PX_NULL)   *blue  = (u8) ((color & 0x000000FF) >> 0);
    if (green != PX_NULL) *green = (u8) ((color & 0x0000FF00) >> 8);
    if (blue != PX_NULL)  *red   = (u8) ((color & 0x00FF0000) >> 16);
    if (alpha != PX_NULL) *alpha = (u8) ((color & 0xFF000000) >> 24);

    return 1;
}

#endif // PX_WIN32_BITMAP_C
